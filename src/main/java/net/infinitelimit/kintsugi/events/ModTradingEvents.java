...
public class ModTradingEvents {
    ...
    private static final Int2ObjectMap<VillagerTrades.ItemListing[]> REPLACEMENT_LIBRARIAN_TRADES =
            new Int2ObjectOpenHashMap<>(ImmutableMap.of(
                    1, new VillagerTrades.ItemListing[] {new VillagerTrades.EmeraldForItems(Items.PAPER, 24, 16, 2), new VillagerTrades.ItemsForEmeralds(Blocks.BOOKSHELF, 7, 1, 12, 1),new VillagerTrades.ItemsForEmeralds(Blocks.CHISELED_BOOKSHELF, 9, 1, 6, 1) },
                    2, new VillagerTrades.ItemListing[] {new VillagerTrades.EmeraldForItems(Items.BOOK, 4, 12, 10), new ModTradeOffers.RandomKnowledgeBookForEmeralds(10), new VillagerTrades.ItemsForEmeralds(Items.LANTERN, 1, 1, 5)},
                    3, new VillagerTrades.ItemListing[] {new VillagerTrades.EmeraldForItems(Items.INK_SAC, 5, 12, 20), new ModTradeOffers.RandomKnowledgeBookForEmeralds(20), new VillagerTrades.ItemsForEmeralds(Items.GLASS, 1, 4, 10)},
                    4, new VillagerTrades.ItemListing[] {new VillagerTrades.EmeraldForItems(Items.WRITABLE_BOOK, 2, 12, 30), new ModTradeOffers.RandomKnowledgeBookForEmeralds(30), new VillagerTrades.ItemsForEmeralds(Items.CLOCK, 5, 1, 15), new VillagerTrades.ItemsForEmeralds(Items.COMPASS, 4, 1, 15)},
                    5, new VillagerTrades.ItemListing[] {new VillagerTrades.ItemsForEmeralds(NAME_TAG, 20, 1, 30), new ModTradeOffers.KnowledgeBookForEmeralds(30)}
            ));

    private static List<VillagerTrades.ItemListing> EXPERIMENTAL_WANDERING_COMMON_TRADES = NonNullList.of(
    ...
    private static List<VillagerTrades.ItemListing> EXPERIMENTAL_WANDERING_RARE_TRADES = NonNullList.of(
    ...
    public static void addCustomTrades(VillagerTradesEvent event) {
        if (event.getType() == VillagerProfession.LIBRARIAN) {
            ...
            // We need to keep the original lists for mutability and mod compatibility
            REPLACEMENT_LIBRARIAN_TRADES.int2ObjectEntrySet().forEach(entry -> {
                List<VillagerTrades.ItemListing> itemListings = trades.get(entry.getIntKey());
                itemListings.clear();
                Collections.addAll(itemListings, entry.getValue());
            });
        }
    }
    ...
    public static void addWanderingTrades(WandererTradesEvent event) {
        ...
        event.getGenericTrades().addAll(EXPERIMENTAL_WANDERING_COMMON_TRADES);
        event.getRareTrades().addAll(EXPERIMENTAL_WANDERING_RARE_TRADES);
    }
